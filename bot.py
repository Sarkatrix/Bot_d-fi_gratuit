...
def charger_defis():
    ...
    return defis[:30]  # Limite à 30 défis pour la version gratuite
...
async def ajoutdefi(interaction: discord.Interaction, defi: str):
    defis = charger_defis_custom()
    if len(defis) >= max_defis_custom:
        await interaction.response.send_message("❌ Limite atteinte : la version gratuite permet seulement 7 défis personnalisés.\n🔓 Débloquez la version complète ici : https://storycraft.bot/premium", ephemeral=True)
        return
...
@bot.tree.command(name="classement", description="Affiche le top 3 des joueurs.")
async def classement(interaction: discord.Interaction):
    ...
    message = "🏆 **Top 3 des défis** :\n"
    for i, (nom, points) in enumerate(classement[:3], start=1):
        message += f"{i}. {nom} - {points} point(s)\n"
    message += "\n🔓 Débloquez le classement complet avec la version premium."
